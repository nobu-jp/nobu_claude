...
├── retirement-simulation.html   # 積立→取り崩しシミュレーター（単体で動くHTML）
└── src/                         # ソースコード（作成予定）
```

### retirement-simulation.html

依存関係なしで開けるシングルファイルのツール。計算ロジックは
`// ==MODEL-START==` 〜 `// ==MODEL-END==` の間にDOM非依存の純粋関数 `calc(p)` として
切り出してあるため、この区間を抜き出せばNode.jsから直接テストできる。
ロジックを変更したときは、このマーカー構造を壊さないこと。
